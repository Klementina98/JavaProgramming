...
class Scheduler<T>{
    ...
    public Timestamp<T> next(){
        Timestamp<?> now= new Timestamp<>(LocalDateTime.now(),"T"); // 11/12/2020
        Collections.sort(list);
        for (Timestamp<T> time:list){
    ...
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        Timestamp<?> Begin=new Timestamp<>(begin,"T");
        Timestamp<?> End=new Timestamp<>(end,"T");
       return list.stream().filter(s->s.compareTo(Begin)>0 & s.compareTo(End)<0).collect(Collectors.toList());
    }
