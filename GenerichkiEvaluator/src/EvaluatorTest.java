...
class Evaluator{
    public static <T extends  Comparable<T>> boolean evaluateExpression (T left, T right, String operator){
        ...
        IEvaluator<T> evaluator = EvaluatorBuider.build(operator); //ova ti vrakja IEEvaluator, bidejki funkcijata build(operator) vrakja IEEvaluator
        return evaluator.evaluate(left,right); //od koga imame evaluator moze da pristapime do evaluate(T a, T b)
