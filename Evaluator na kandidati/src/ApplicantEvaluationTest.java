...
class EvaluatorBuilder{
    public static Evaluator build(Evaluator.TYPE type) throws InvalidEvaluation {
        ...
        // вашиот код овде
        Evaluator MORE_EXPERIENCE=new Evaluator() {
            @Override
            public boolean evaluate(Applicant applicant) {
                return applicant.getEmploymentYears()>10;
            }
        };
        Evaluator MORE_CREDIT_SCORE = applicant -> applicant.getCreditScore()>500;
        Evaluator NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE = applicant -> !applicant.hasCriminalRecord() && applicant.getEmploymentYears()>10;
        Evaluator MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE  = applicant -> applicant.getEmploymentYears()>10 && applicant.getCreditScore()>500;
        Evaluator NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE =applicant -> !applicant.hasCriminalRecord() && applicant.getCreditScore()>500;

        if (type.equals(Evaluator.TYPE.MORE_EXPERIENCE)) return MORE_EXPERIENCE;
        if (type.equals(Evaluator.TYPE.MORE_CREDIT_SCORE)) return MORE_CREDIT_SCORE;
        if (type.equals(Evaluator.TYPE.NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE)) return NO_CRIMINAL_RECORD_AND_MORE_EXPERIENCE;
        if (type.equals(Evaluator.TYPE.MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE)) return MORE_EXPERIENCE_AND_MORE_CREDIT_SCORE;
        if (type.equals(Evaluator.TYPE.NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE)) return NO_CRIMINAL_RECORD_AND_MORE_CREDIT_SCORE;
